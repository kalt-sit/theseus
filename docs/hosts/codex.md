...
Make the candidate available as a local directory or identify an immutable repository revision. A suitable request is:
...
A Skill cannot enforce Codex sandbox or approval settings. Confirm the active mode before the audit begins. For an audit-only task, do not grant write access or authorize installation or external transmission of candidate content.
